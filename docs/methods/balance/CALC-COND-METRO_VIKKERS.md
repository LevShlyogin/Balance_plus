# [CALC-COND-METRO_VIKKERS] Методика расчета давления в конденсаторе (Метро-Виккерс)
...
## 1. Назначение
Алгоритм вычисляет абсолютное давление пара перед конденсатором ($P_k$) на основе геометрических характеристик оборудования и параметров охлаждающей воды. Используется методика Метро-Виккерса, основанная на определении коэффициента теплопередачи.
...
| :--- | :--- | :---: | :---: | :--- |
| Внутренний диаметр труб | `diameter_inside_of_pipes` | $d_{vn}$ | мм | |
| Толщина стенки трубы | `thickness_pipe_wall` | $s_{tr}$ | мм | |
| Активная длина труб | `length_cooling_tubes_...` | $L_{act}$ | м | Длина труб основного пучка |
| Число труб осн. пучка | `number_cooling_tubes_of_the_main_bundle` | $N_{op}$ | шт | |
| Число труб встр. пучка | `number_cooling_tubes_of_the_built-in_bundle` | $N_{vp}$ | шт | |
| Число труб ВО | `number_air_cooler_total_pipes` | $N_{vo}$ | шт | Трубки воздухоохладителя (опционально) |
| Число ходов воды | `number_cooling_water_passes...` | $Z_{op}$ | шт | Число ходов в основном пучке |

### 4.2. Термодинамические параметры и Свойства
| Параметр | Обозначение (Код) | Символ | Единица | Описание |
| :--- | :--- | :---: | :---: | :--- |
| Расход охл. воды | `mass_flow_cooling_water` | $G_{ov}$ | т/ч |  |
| Температура воды (вход) | `temperature_cooling_water_1` | $t_{ov1}$ | °С | |
| Коэфф. чистоты | `coefficient_b` | $\beta$ | - | Коэффициент, учитывающий загрязнение (по умолч. 1.0) |
| Теплопроводность | `thermal_conductivity_...` | $\lambda$ | Вт/(м·К) | Теплопроводность материала трубок |
| Расход пара в конденсатор | `mass_flow_flow_path_1` | $G_1$ | т/ч | Расход пара из проточной части |
| Степень сухость | `degree_dryness_flow_path_1` | $X_1$ | - | Степень сухости пара на входе |
...
| :--- | :--- | :---: | :---: |
| Теплота парообразования | `WaterProps` / Формула | $r_{par} = f(t_{ov1})$ | ккал/кг (или кДж/кг) |
| Давление насыщения | `tsat2p` | $P = f(t_{sat})$ | кгс/см² (или МПа) |

## 6. Алгоритм расчета

### Шаг 0. Предварительная обработка
Если $N_{vo}$ не задано, вычисляем:

$$ N_{vo} = (N_{op} + N_{vp}) \cdot 0.15 $$

### Шаг 1. Геометрические характеристики
Наружный диаметр трубок ($d_{out}$):

$$ d_{out} = d_{vn} + 2 \cdot s_{tr} $$

Полная площадь поверхности трубного пучка ($F_{total}$):

$$ F_{total} = \pi \cdot L_{act} \cdot (N_{op} + N_{vp}) \cdot d_{out} $$

> Примечание:
> 1. Параметры $L_{act}$ и $d_{out}$ переводим в [м]
> 2. Для валидации используем число $pi=3.14159$

Площадь поверхности воздухоохладителя ($F_{vo}$):

$$ F_{vo} = \pi \cdot L_{act} \cdot N_{vo} \cdot d_{out} $$
...
$$ R_1 = \frac{2 \cdot s_{tr} \cdot d_{out}}{(d_{out} + d_{vn}) \cdot \lambda} $$

> Примечание:
> 1. Параметры $s_{tr}$, $d_{out}$ и $d_{vn}$ переводим в [м]
...
$$ C_{ov} = \frac{G_{ov} \cdot Z_{op}}{900 \cdot \pi \cdot (N_{op} + N_{vp}) \cdot d_{vn}^2} $$

Теплота парообразования ($r_{par}$) определяется через IF97 $r_{par} = f(t_{ov1})$

> Примечание:
> 1. Для валидации используем уравнение
> 
> $$ r_{par} = (30 - t_{ov1}) \cdot 0.582 + 580.4 $$

Температура воды на выходе ($t_{ov2}$):

$$ t_{ov2} = t_{ov1} + \frac{G_1 \cdot r_{par} \cdot X_1}{G_{ov}} $$

Средняя температура воды ($t_{avg}$):

$$ t_{avg} = \frac{t_{ov1} + t_{ov2}}{2} $$
...
$$ dt_{und} = \frac{1}{e^{\left( \frac{K_{dirty} \cdot F_{total}}{G_{ov}} \right)} - 1} $$

> Примечание:
> 1. Параметр $G_{ov}$ переводим в [кг/ч]

### Шаг 7. Расчет давления
Температура насыщения ($t_{sat}$):

$$ t_{sat} = t_{ov2} + dt_{und} \cdot (t_{ov2} - t_{ov1}) $$
