...
def print_beautifully(data_dict, friendly_names, title, keys_to_print):
    ...
    """
    print(f"\n--- {title} ---")
    max_len = max(len(f"{friendly_names[k][0]} [{friendly_names[k][1]}]") for k in keys_to_print if k in friendly_names)
...
def generate_and_print_tables(strategy, base_params):
    ...
    for beta in beta_values:
        print("\n" + "="*60)
        print(f"4. Результаты для построения графиков")
...
def main():
    ...
    friendly_names = {
        'diameter_inside_of_pipes': ('Внутренний диаметр трубок', 'мм'), 'thickness_pipe_wall': ('Толщина стенки трубок', 'мм'),
        'length_cooling_tubes_of_the_main_bundle': ('Активная длина труб', 'м'), 'number_cooling_tubes_of_the_main_bundle': ('Кол-во трубок осн. пучка', 'шт'),
        'number_cooling_tubes_of_the_built_in_bundle': ('Кол-во трубок встр. пучка', 'шт'), 'number_cooling_water_passes_of_the_main_bundle': ('Число ходов воды (Z)', 'шт'),
        'mass_flow_cooling_water': ('Расход охлаждающей воды', 'т/ч'), 'temperature_cooling_water_1': ('Температура воды на входе', '°C'),
        'thermal_conductivity_cooling_surface_tube_material': ('Теплопроводность материала', 'Вт/(м·К)'), 'coefficient_b': ('Коэффициент чистоты (β)', '-'),
        'mass_flow_flow_path_1': ('Расход пара', 'т/ч'), 'degree_dryness_flow_path_1': ('Степень сухости пара', '-'),
        'diameter_outside_of_pipes': ('Наружный диаметр трубок', 'мм'), 'area_tube_bundle_surface_total': ('Площадь поверхности осн. пучка', 'м²'),
        'area_surface_of_the_air_cooler_tube_bundle': ('Площадь поверхности возд-ля', 'м²'), 'coefficient_Kf': ('Коэффициент отношения площадей', '-'),
        'coefficient_R1': ('Терм. сопротивление стенки', 'м²·К/Вт'), 'speed_cooling_water': ('Скорость охлаждающей воды', 'м/с'),
        'heat_of_vaporization': ('Теплота парообразования', 'ккал/кг'), 'temperature_cooling_water_2': ('Температура воды на выходе', '°C'),
        'temperature_cooling_water_average_heating': ('Средняя температура воды', '°C'), 'coefficient_K_temp': ('К-т теплопередачи (подобранный)', 'Вт/(м²·К)'),
        'coefficient_K': ('К-т теплопередачи (чистый)', 'Вт/(м²·К)'), 'coefficient_R': ('Терм. сопротивление загрязнений', 'м²·К/Вт'),
        'coefficient_Kzag': ('К-т теплопередачи (с загрязнением)', 'Вт/(м²·К)'), 'temperature_relative_underheating': ('Относительный недогрев', '°C'),
        'temperature_saturation_steam': ('Температура насыщения', '°C'), 'pressure_flow_path_1': ('Давление пара в конденсаторе', 'кгс/см²'),
    }
    ...
    try:
        ...
        intermediate_keys = [
            'speed_cooling_water', 'temperature_cooling_water_2', 'temperature_cooling_water_average_heating',
            'coefficient_K_temp', 'temperature_saturation_steam'
        ]
